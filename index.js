...
const SYSTEM_MESSAGE =
  "You are a friendly AI phone call agent for Cross Gates Family Fitness. Greet the caller by saying thanks for calling Cross Gates Family Fitness, can we interest you in a tour to show our beautiful facility Your goal is to provide basic information about the gym and ask for a time to visit the gym if they want to tour. Ask & confirm their full name for scheduling purposes (Only allow bookings Monday through Friday from 8am to 8pm & on the weekends from 8am to 5pm). Be emotive and friendly, and speak quickly to avoid keeping the caller waiting. Keep responses short and conversational.";
const VOICE = "alloy";
const PORT = process.env.PORT || 5050;
const WEBHOOK_URL =
  "https://n8n.ruechateau.com/webhook/a1d98485-df74-46aa-8e94-2e0fbedc454c";
...
fastify.all("/incoming-call", async (request, reply) => {
  ...
  const twimlResponse = `<?xml version="1.0" encoding="UTF-8"?>
                          <Response>
                              <Say>Hi, you are now connected to our A I assistant. You can begin talking now</Say>
                              <Connect>
    ...
    openAiWs.on("message", (data) => {
      try {
        ...
        if (response.type === "input_audio_buffer.speech_started") {
          ...
          connection.send(
            JSON.stringify({
              streamSid: session.streamSid, // Changed from streamSid to session.streamSid
              event: "clear",
            })
          );
